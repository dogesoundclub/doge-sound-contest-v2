...
contract DogeSounds is KIP17Full("DogeSoundClub DogeSounds", "DOGESOUNDS"), KIP17Mintable, KIP17Burnable, KIP17Pausable {

    function tokenURI(uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId), "KIP17Metadata: URI query for nonexistent token");
        
        if (tokenId == 0) {
            return "https://api.dogesound.club/dogesounds/0";
        }

        string memory baseURI = "https://api.dogesound.club/dogesounds/";
        string memory idstr;
